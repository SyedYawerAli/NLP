...
def remove_from_order(parameters: dict, session_id: str):
    ...
    if len(food_items) != len(quantities):
        return JSONResponse(content={
            "fulfillmentText": "Sorry, I couldn't understand the quantities. Please specify the food items and quantities clearly."
        })
    ...
    for item, quantity in zip(food_items, quantities):
        ...
        if item not in current_order:
            ...
            if current_order[item] <= quantity:
                ...
                removed_items.append(f"all of {item}")
                del current_order[item]
            else:
                # Reduce the quantity of the item in the order
                current_order[item] -= quantity
                removed_items.append(f"{quantity} of {item}")

    # Construct the response message
    fulfillment_text = ""
    if removed_items:
        fulfillment_text += f"Removed: {', '.join(removed_items)}. Anything Else?"
